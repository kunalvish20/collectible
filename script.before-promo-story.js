(() => {
  ...
  function updateScrollScene() {
    ...
    if (!prefersReducedMotion && storySequence && storyVisual) {
      ...
      // Stage 0: the two visual cards rise into the viewport first.
      const visualIn = clamp(p / .14);
      const visualDrift = clamp(p / .28);
      ...
      // Stage 1: copy card comes from below and completely covers both images.
      const copyIn = clamp((p - .16) / .20);
      // Stage 2 and 3 repeat the same cover interaction.
      const peopleIn = clamp((p - .45) / .19);
      const mediaIn = clamp((p - .73) / .19);
